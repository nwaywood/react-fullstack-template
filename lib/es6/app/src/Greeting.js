// Generated by BUCKLESCRIPT VERSION 2.0.0, PLEASE EDIT WITH CARE
'use strict';

import * as Utils       from "./Utils.js";
import * as React       from "react";
import * as MessageRe   from "./MessageRe.js";
import * as GreetingTwo from "./GreetingTwo.js";
import * as ReasonReact from "reason-react/lib/es6/src/reasonReact.js";

var component = ReasonReact.statelessComponent("Greeting");

function make() {
  var newrecord = component.slice();
  newrecord[/* render */9] = (function () {
      return React.createElement("div", undefined, React.createElement("button", undefined, Utils.textEl("Hello!")), ReasonReact.element(/* None */0, /* None */0, GreetingTwo.make(/* array */[])), ReasonReact.element(/* None */0, /* None */0, MessageRe.make("Test", /* array */[])));
    });
  return newrecord;
}

var $$default = ReasonReact.wrapReasonForJs(component, (function () {
        return make(/* array */[]);
      }));

export {
  component ,
  make      ,
  $$default ,
  $$default   as default,
  
}
/* component Not a pure module */
